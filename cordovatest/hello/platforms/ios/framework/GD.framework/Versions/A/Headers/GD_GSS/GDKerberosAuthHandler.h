 ...
 * obtained in the course of its secure communication with application servers.
 * This C++ class contains a number of functions for managing tickets in the
 * cache, and for setting default ticket request parameters and configuration.
 *
 * This class supports Kerberos delegation in a number of forms. However, use of
 * this class alone doesn't necessarily result in delegation being used in all
 * cases. The use of delegation can also depend on configuration in the Kerberos
 * Key Distribution Center (KDC) and in the BlackBerry Dynamics management
 * console.
 */
class GD_ATTRIBUTE((visibility("default"))) GDKerberosAuthHandler  /* final */
{
...
public:
     ...
     * allowed or disallowed, within BlackBerry Dynamics secure communication.
     *
     * Kerberos authentication delegation can be allowed and disallowed by
     * calling the \reflink GDKerberosAuthHandler::setAllowDelegation
     * setAllowDelegation\endlink function, below.
     *
     * @return <tt>true</tt> if Kerberos delegation is allowed within BlackBerry
 ...
 * Kerberos delegation work in the whole end-to-end application.
    ...
    /** Get a Kerberos ticket with a user name and password, and specify whether
     *  it can be delegated.
     * 
     * Call this function to create an initial Kerberos ticket for
     * authentication. This will be a Ticket-to-Get-Tickets (TGT), for a
     * specified user principal. If the ticket is created, it will be stored in
     * the BlackBerry Dynamics secure cache.
     *
     * Specify in the <tt>allowDelegation</tt> parameter whether to request a
     * ticket that can be delegated. Even if a ticket that can be delegated is
     * requested, a ticket that cannot be delegated might be issued, depending
     * on the configuration of the Kerberos KDC.
     *
     * BlackBerry Dynamics secure communication supports Kerberos authentication
     * of only one active user principal at a time.
     *
     * The user principal name must be in the
     * <em>user</em><tt>\@</tt><em>realm</em> long form. The short form
     * <em>shortrealm</em><tt>\\</tt><em>user</em> isn't supported.
     *
     ...
     *                 null terminator.
     *
     * @param allowDelegation <tt>bool</tt> for whether to request a ticket that
     *                        can be delegated.
     *
     ...
     *
     * Calling this function is equivalent to calling the following.\code
    ...
    /** Get a Kerberos ticket via PKINIT authentication.
     * 
     * Call this function to create an initial Kerberos ticket for
     * authentication. This will be a Ticket-to-Get-Tickets (TGT), for an
     * implicit user principal authenticated by using public key cryptography
     * for initial Kerberos authentication (PKINIT). If the ticket is created,
     * it will be stored in the BlackBerry Dynamics secure cache.
     *
     * This function depends on the enterprise having deployed PKINIT
     * authentication.
     *
     * Specify in the <tt>allowDelegation</tt> parameter whether to request a
     * ticket that can be delegated. Even if a ticket that can be delegated is
    ...
    /** Get a Kerberos ticket via Kerberos Constrained Delegation, with implicit
     *  credentials, and specify whether it can be delegated.
     * 
     ...
     * (KCD) to a specified authentication host, with implicit credentials. If
     * the ticket is created, it will be stored in the BlackBerry Dynamics
     * secure cache.
     *
     ...
     *
     * Specify in the <tt>allowDelegation</tt> parameter whether to request a
     * ticket that can itself be delegated. Even if a ticket that can be
     * delegated is requested, a ticket that cannot be delegated might be
     * issued, depending on the configuration of the Kerberos KDC.
     * 
    ...
    /** Check whether implicit credentials are allowed.
     * 
     * Call this function to check whether implicit credentials are allowed.
     *
     * If implicit credentials are allowed, then the following ticket set-up
     * variants can be used.
     * - \ref setUpKerberosTicket(const char* host, int port, bool allowDelegation)
     * - \ref setUpKerberosTicket(const char* host, int port)
     * .
     * In those variants, a ticket is obtained by Kerberos Constrained
     * Delegation (KCD).
     * 
