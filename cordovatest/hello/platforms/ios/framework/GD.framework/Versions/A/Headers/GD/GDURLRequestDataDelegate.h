 ...
 * - Callback functions are only invoked if the BlackBerry Dynamics proxy
 *   infrastructure is in use via the URL Loading System, i.e. only if the 
 ...
 * 
 * @see <a
 *          HREF="https://developer.apple.com/reference/foundation/nsurlconnectiondatadelegate?language=objc"
 *          target="_blank"
 *      >NSURLConnectionDataDelegate protocol reference</a
 *      > on the apple.com developer website.
 * 
 * @see <a
 *          HREF="https://developer.apple.com/reference/uikit/uiwebviewdelegate?language=objc"
 *          target="_blank"
 *      >UIWebViewDelegate protocol reference</a
 *      > on the apple.com developer website.
 */
 ...
 * <tt>NSURLResponse</tt> object. For details of what information is available
 * and how to access it, see the <a
 *     HREF="https://developer.apple.com/reference/foundation/nsurlrequest?language=objc"
 *     target="_blank"
 ...
 *
 * The received data is made available in an <tt>NSData</tt> object. For details
 * of how to read it, see the <a
 *     HREF="https://developer.apple.com/reference/foundation/nsdata?language=objc"
 *     target="_blank"
 * >NSData class reference</a> on the apple.com developer website.
 * 
 * The response data may have been uncompressed after transport and its final
 * length may be different to the expected length when
 * \ref GDRequest:didReceiveResponse: was invoked.
 * 
 * The response data for a single request may be received in multiple chunks, in
 * which case this callback will be invoked multiple times. This callback won't
 * be invoked for responses that include no data.
 *
 * @param request <tt>NSURLRequest</tt> representing the request for which
 ...
 * stopped and cannot be resumed. The application would have to reload the page
 * from the start, after the error condition had been resolved.
 *
 * Details of the error condition are made available in an <tt>NSError</tt>
 * object. For details of how to access its attributes, see the <a
 *     href="https://developer.apple.com/reference/foundation/nserror?language=objc"
 *     target="_blank"
...
/** Invoked to check whether loading should continue when a request fails or an
 *  incomplete response is received.
 * 
 * This callback is invoked when a URL request sent by the associated
 * <tt>UIWebView</tt> instance fails to send, or receives an incomplete
 * response. The function that is invoked can select to continue loading the
 * page, possibly with a modified request, or can select to stop loading the
 * page.
 *
 * Details of the error condition are made available in an <tt>NSError</tt>
 * object. For details of how to access its attributes, see the <a
 ...
 * 
 * If the invoked function determines that the error can be resolved, then
 * the function should return <tt>YES</tt>. If the error cannot be resolved,
 * then the function should return <tt>NO</tt> instead.
 *
